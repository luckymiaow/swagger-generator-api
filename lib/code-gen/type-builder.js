...
export function buildTypeRef(schema, types, schemas) {
    const typeRef = buildType(schema, types, schemas);
    const nullable = schema.nullable;
    return { typeRef, nullable };
}
export function buildType(obj, types, schemas) {
    if ('type' in obj) {
        const schema = obj;
        if (schema.builtType)
...
export function findOrBuildType(typeRef, obj, types, schemas) {
    if (typeRef in types)
...
export function buildTypes(schemas) {
    const types = {};
