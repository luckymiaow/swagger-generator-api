...
function getAction(actionName, item) {
    return {
        url: item.path,
        method: item.method,
        name: actionName,
...
function fetchControllers(nodes, tagObj) {
    ...
    for (const [key, value] of entroes) {
        ...
        controllers.push({
            ...
            actions: actions?.map(([actionName, item]) => getAction(actionName, item)),
        });
    }
    return controllers;
}
function fetchApis(doc, definedTypes) {
    ...
    for (const [key, apis] of groupApi) {
        ...
        if (typeof isClass[0][1] === 'string') {
            res.actions?.push(getAction(key, apis));
        }
...
async function generateApisAsync(setting, doc, definedTypes, models) {
    ...
    if (!setting.template.api.transform)
        setting.template.api.transform = presets_1.defaultApisTransform;
    if (typeof setting.template.api.transform === 'string') {
        ...
        const fileId = (0, utils_1.getFileId)(setting.basePath, apiConfig.output, 'index', 'apis');
        await (0, utils_1.writeFileWithDirectoryCreation)(fileId, apiConfig.prettier !== false ? (0, utils_1.prettierCode)(code) : code);
        paths.index = fileId;
    }
    else {
        const fileId = (0, utils_1.getFileId)(setting.basePath, undefined, 'index', 'apis');
        ...
        for (const item of cyclicBody) {
            ...
            if (!item.output || typeof item.output === 'string')
                fileId = (0, utils_1.getFileId)(setting.basePath, item.output, 'index', 'apis');
            else
                fileId = item.output((0, utils_1.getFileId)(setting.basePath, undefined, 'index', 'apis'));
            await (0, utils_1.writeFileWithDirectoryCreation)(fileId, setting.template.api.prettier !== false ? (0, utils_1.prettierCode)(item.code) : item.code);
