...
function flattenOperations(node, prefix = '') {
    const result = [];
    for (const [k, v] of Object.entries(node)) {
        ...
        if (isApiOperation(v))
            ...
            result.push(...flattenOperations(v, name));
    }
    return result;
}
function fetchControllers(nodes, tagObj, setting, model) {
    const controllers = [];
    for (const [key, value] of Object.entries(nodes)) {
        const ops = flattenOperations(value);
        controllers.push({
            name: key,
...
function fetchApisAsync(doc, definedTypes, setting, model) {
    ...
    for (const apiPath in doc.paths) {
        ...
        const urlParts = apiPath
            .split(/[/\\]/)
            .filter(s => !typescript_string_operations_1.String.IsNullOrWhiteSpace(s))
            .map((s) => {
        ...
        while (actionIdx >= 0 && urlParts[actionIdx].isParam)
            actionIdx--;
...
function groupApis(apiRoot, tagObj, setting, model) {
    ...
    if (groupApi.length === 1) {
        ...
        if (isApiOperation(value))
            res.actions?.push(getAction(key, value, setting, model));
        else
            res.controllers = fetchControllers(groupApi[0][1], tagObj, setting, model);
        return res;
    }
    for (const [key, apis] of groupApi) {
        if (isApiOperation(apis)) {
            ...
            res.controllers?.push(...fetchControllers({ [key]: apis }, tagObj, setting, model));
        }
        else {
            res.namespaces?.push({
                name: key,
                controllers: fetchControllers(apis, tagObj, setting, model),
                description: tagObj?.[key],
            });
        }
    }
