...
async function generateModelsAsync(types, setting) {
    ...
    const modelsPath = {};
    let dtsPath = '';
    if (!setting.template.models.transform)
        setting.template.models.transform = presets_1.defaultModelTransform;
    if (typeof setting.template.models.transform === 'string') {
        models.forEach((model) => {
            const code = handlebarsTransform(setting.template.models.transform, model);
            const fileId = (0, utils_1.getModelsFileId)(setting, model.key);
            (0, utils_1.writeFileWithDirectoryCreation)(fileId, setting.template.models.prettier !== false ? (0, utils_1.prettierCode)(code) : code);
...
function handlebarsTransform(text, data) {
    const template = handlebars_1.default.compile(text, { noEscape: false });
