...
exports.fetchModelsAsync = exports.getModelByIDotnetType = exports.generateModelsAsync = void 0;
const handlebars_1 = __importDefault(require("handlebars"));
const presets_1 = require("../presets");
const utils_1 = require("./utils");
function generateModelsAsync(models, setting) {
    const modelsPath = {};
    let dtsPath = '';
    ...
    const modelDir = models.reduce((a, b) => ((a[b.name] = b), a), {});
    for (let model of models) {
        if (modelsOption.onBeforeWriteFile)
            model = modelsOption.onBeforeWriteFile(model, models, modelDir);
        let code = presets_1.defaultModelTransform;
        if (!modelsOption.transform || typeof modelsOption.transform === 'string')
            code = handlebarsTransform(modelsOption.transform, model);
        else
            code = modelsOption.transform(model);
...
function getModelByIDotnetType(type, key) {
    const isClass = type.isInterface === false;
    const item = {
        definition: isClass ? 'class' : 'interface',
        name: (0, utils_1.makeModelName)(type),
        key,
        dependencys: [],
        properties: [],
    };
...
exports.getModelByIDotnetType = getModelByIDotnetType;
